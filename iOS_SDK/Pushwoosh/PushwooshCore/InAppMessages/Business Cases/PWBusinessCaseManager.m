        ...
        if (!businessCase) {
            ...
            if (!businessCase) {
                [PushwooshLog pushwooshLog:PW_LL_WARN
                                 className:self
                                   message:[NSString stringWithFormat:@"Unknown business case %@", identifier]];
                return;
            }
            ...
            [PushwooshLog pushwooshLog:PW_LL_INFO
                             className:self
                               message:@"Business case is already active"];
        }
    }
}
...
+ (NSDictionary *)businessCasesDictionary {
    ...
    increasePushRateCase.conditionBlock = ^BOOL {
        return [PWManagerBridge shared].getPushToken == nil && ![PWPreferences preferences].registrationEverOccured;
    };
    ...
    recoveryPushCase.conditionBlock = ^BOOL {
        NSDictionary *dict = [PWManagerBridge getRemoteNotificationStatus];
